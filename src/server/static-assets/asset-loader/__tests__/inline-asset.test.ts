  ...
  describe('createWasmInlineAsset', () => {
    ...
    describe('getStoreEntry method', () => {
      it('should return embedded store entry with correct data', async () => {
        ...
        const asset = createWasmInlineAsset(metadata);
        const storeEntry = await asset.getStoreEntry('test-asset');
      ...
      it('should handle multiple calls to getStoreEntry', async () => {
        expect.assertions(3);
        const metadata = createTestMetadata();
        const mockFileContent = 'Hello, World! This is test data.';
        const mockStoreEntry = createTestFileBinaryArray(mockFileContent);
        ...
        const asset = createWasmInlineAsset(metadata);
        const storeEntryFirst = await asset.getStoreEntry('test-asset');
        const storeEntrySecond = await asset.getStoreEntry('test-asset');
      ...
      if (runSlowTests) {
        it('should handle large files [slow]', async () => {
          ...
          const asset = createWasmInlineAsset(metadata);
          await asset.getStoreEntry('test-asset');
